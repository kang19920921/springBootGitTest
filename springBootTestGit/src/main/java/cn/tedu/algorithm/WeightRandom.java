...
//            Preconditions.checkArgument(pair.getValue().doubleValue() > 0, String.format("非法权重值：pair=%s", pair));
    ...
    public K random() {
        ...
        SortedMap<Double, K> tailMap = this.weightMap.tailMap(randomWeight, false);// 大于等于randomWeight部分map
        return this.weightMap.get(tailMap.firstKey());// 返回当前map中最小的key
    }
